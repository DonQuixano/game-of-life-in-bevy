...
use bevy::{asset::processor::InitializeError, color, ecs::query, gizmos::grid, input::mouse, prelude::*, render::mesh, text::cosmic_text::ttf_parser::Width};
#[cfg(not(target_arch = "wasm32"))]
use std::{cell::{self, Cell}, sync::{self, Arc}, vec};
...
fn setup(
    ...
    window.resolution.set((WIDTH as f32) * 10.0, (HEIGHT as f32) * 10.0);
   ...
   // let mut shapes: Vec<Handle<Mesh>> = vec![];
    ...
    for (i, shape) in Pixels.grid.iter().enumerate() {
        ...
        let matt_id = matt.id();
        let pixel:Entity = commands.spawn((
            Mesh2d(shape.clone()),
            MeshMaterial2d(matt),
            Transform::from_xyz(
...
fn game_loop(mut commands: Commands,
    mut meshes: ResMut<Assets<Mesh>>,
    mut materials: ResMut<Assets<ColorMaterial>>,
    mut Pixels: ResMut<Pixels>, mut CellGrid: ResMut<CellGrid>,
    mut Entities: ResMut<Entities>,
    mut Matts: ResMut<Matts>,
    mut InititalRun: ResMut<InititalRun>,
    ...
    let scangrid: Vec<u8> = CellGrid.arr.clone();
        ...
        n_count += if scangrid[index_map(cell_x + 1, cell_y + 1, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0};
        n_count += if scangrid[index_map(cell_x - 1, cell_y - 1, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0};
        n_count += if scangrid[index_map(cell_x - 1, cell_y + 1, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0};
        n_count += if scangrid[index_map(cell_x + 1, cell_y -1, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0}; 

        n_count += if scangrid[index_map(cell_x + 1, cell_y, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0};
        n_count += if scangrid[index_map(cell_x - 1, cell_y, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0};
        n_count += if scangrid[index_map(cell_x , cell_y + 1, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0};
        n_count += if scangrid[index_map(cell_x , cell_y - 1, WIDTH as i32, HEIGHT as i32)] == 1 {1} else {0};
    ...
    if buttons.just_pressed(MouseButton::Left) && mousexy.paused {
        ...
        loop {
            if let Some(material) = materials.get_mut(Matts.grid[the_index]) {
                material.color = if grid.arr[the_index] == dummy {OldColor} else {NewColor};
                break;
            }}
    }
